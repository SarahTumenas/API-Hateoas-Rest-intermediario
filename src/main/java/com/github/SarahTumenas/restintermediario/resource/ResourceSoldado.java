...
public class ResourceSoldado {
    ...
    public SoldadoListResponse criarLink(SoldadoEntity soldadoEntity){
        ...
        Link link = WebMvcLinkBuilder.linkTo(methodOn(SoldadoController.class).buscarSoldado(soldadoEntity.getId())).withSelfRel();
    ...
    public SoldadoResponse criarLinkDetalhe(SoldadoEntity soldadoEntity){
        ...
        if (soldadoEntity.getStatus().equals("Morto")){
            link = WebMvcLinkBuilder.linkTo(methodOn(SoldadoController.class).deletarSoldado(soldadoEntity.getId()))
                    .withRel("remover")
                    .withTitle("Deletar soldado")
                    .withType("delete");
        } else if (soldadoEntity.getStatus().equals("Vivo")) {
            link = WebMvcLinkBuilder.linkTo(methodOn(SoldadoController.class).patrulharCastelo(soldadoEntity.getId()))
                    .withRel("patrulha")
                    .withTitle("Patrulhar Castelo")
                    .withType("put");
        } else {
            link = WebMvcLinkBuilder.linkTo(methodOn(SoldadoController.class).buscarSoldado(soldadoEntity.getId())).withSelfRel();
        }
